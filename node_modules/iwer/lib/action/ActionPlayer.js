...
import { P_ACTION_PLAYER, P_GAMEPAD, P_JOINT_SPACE, P_SPACE, } from '../private.js';
...
export class ActionPlayer {
    constructor(refSpace, recording, ipd, options = {}) {
        ...
        mat4.fromTranslation(this[P_ACTION_PLAYER].viewSpaces[XREye.Left][P_SPACE].offsetMatrix, vec3.fromValues(-ipd / 2, 0, 0));
        mat4.fromTranslation(this[P_ACTION_PLAYER].viewSpaces[XREye.Right][P_SPACE].offsetMatrix, vec3.fromValues(ipd / 2, 0, 0));
    ...
    updateXRSpaceFromMergedFrames(space, lastTransform, nextTransform, alpha) {
        const f1p = vec3.set(this[P_ACTION_PLAYER].f1p, lastTransform[0], lastTransform[1], lastTransform[2]);
        const f1q = quat.set(this[P_ACTION_PLAYER].f1q, lastTransform[3], lastTransform[4], lastTransform[5], lastTransform[6]);
        const f2p = vec3.set(this[P_ACTION_PLAYER].f2p, nextTransform[0], nextTransform[1], nextTransform[2]);
        const f2q = quat.set(this[P_ACTION_PLAYER].f2q, nextTransform[3], nextTransform[4], nextTransform[5], nextTransform[6]);
        vec3.lerp(this[P_ACTION_PLAYER].vec3, f1p, f2p, alpha);
        quat.slerp(this[P_ACTION_PLAYER].quat, f1q, f2q, alpha);
        mat4.fromRotationTranslation(space[P_SPACE].offsetMatrix, this[P_ACTION_PLAYER].quat, this[P_ACTION_PLAYER].vec3);
    }

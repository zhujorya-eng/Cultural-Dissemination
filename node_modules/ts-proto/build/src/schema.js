...
const fileDescriptorProto = (0, ts_poet_1.imp)("t:FileDescriptorProto@ts-proto-descriptors");
  ...
  `);
    ...
    (0, visit_1.visitServices)(fileDesc, sourceInfo, (serviceDesc) => {
        if (options.outputClientImpl) {
            const suffix = options.outputServices.includes(options_1.ServiceOption.GRPC) ? "Client" : "ClientImpl";
            addReference(serviceDesc.name, `${serviceDesc.name}${suffix}`);
        }
    });
    const dependencies = fileDesc.dependency.map((dep) => {
        return (0, ts_poet_1.code) `${(0, utils_1.impFile)(options, `protoMetadata@./${dep.replace(".proto", "")}${options.fileSuffix}`)}`;
    });
